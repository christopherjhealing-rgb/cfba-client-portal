 ...
 *
 * The rule, as CFBA applies it (the pre-2016 exemption covers BOTH kinds —
 * a house built before 2016 was never BAL-assessed, so structures beside it
 * are exempt):
 *   • 6 m or more from the house → nothing.
 *   • within 6 m, house before 2016 → exempt, shed and patio alike.
 *   • within 6 m, house 2016 or later, a shed → its own new BAL report.
 *   • within 6 m, house 2016 or later, a patio or carport → the house's
 *       BAL rating, evidenced.
 *
  ...
  if (kind === "shed") {
    if (age === "pre2016") {
      return {
        ...
        detail: "The house was built before 2016, so it was never BAL-assessed and the shed is exempt.",
        summary: "BAL: exempt — shed on a house built before 2016.",
    ...
    if (age === "post2016") {
      return {
        ...
        detail: "A shed within 6 m of a house built in 2016 or later is assessed in its own right, not off the house's rating. We don't prepare BAL reports — attach the shed's new report in the box below; the job can't lodge without it.",
        summary: "BAL: shed within 6 m, house 2016 or later — needs its own new BAL report.",
        mondayBal: null,
      };
    }
    if (age === "unsure") {
      return {
        ...
        detail: "If the house was built before 2016 the shed is exempt; if it's newer the shed needs its own new BAL report. Lodge the job and we'll let you know which.",
        summary: "BAL: shed within 6 m, house age unknown — to confirm on assessment.",
        mondayBal: null,
      };
    }
    return null;
