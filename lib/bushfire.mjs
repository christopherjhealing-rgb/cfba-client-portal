 ...
 * `summary` is the one line filed with the lodgement so the office sees the
 * answer without re-asking. Pure — see tests/bushfire.test.mjs.
 */
export function balVerdict({ distance, kind, age } = {}) {
  if (distance === "far") {
    return {
      tone: "clear",
  ...
  if (kind === "shed") {
    return {
      ...
      detail: "A shed within 6 m of the house is assessed in its own right, not off the house's rating. We don't prepare BAL reports — once you have it, attach it under Other Supporting Documents, or lodge now and we'll confirm.",
  ...
  if (age === "post2016") {
    return {
      ...
      detail: "A patio or carport within 6 m takes the house's BAL rating. Attach evidence of it — the original BAL report or certificate, the house's Certificate of Design Compliance, or the original house plans if it's noted on them — under Other Supporting Documents. We don't prepare BAL reports ourselves.",
      summary: "BAL: patio/carport within 6 m, house built 2016 or later — needs the house's BAL evidence.",
    };
  }
  if (age === "unsure") {
    return {
