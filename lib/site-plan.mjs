...
export function sanitisePatio(raw) {
  ...
  const soak = p.soak && typeof p.soak === "object" && typeof p.soak.key === "string"
    ...
    : null;
  const sheeting = typeof p.sheeting === "string" && p.sheeting.trim()
    ? p.sheeting.trim().slice(0, 40) : "Custom Orb";
  const floorRaw = Number(p.floorAbove);
  const floorAbove = Number.isFinite(floorRaw)
    ? Math.round(Math.min(Math.max(floorRaw, 0), 2) * 1000) / 1000 : 0;
  return {
    ...
    soak,
    // Roof sheeting (material) named on the sheet, and the patio floor's height
    // above natural ground — the level figures a lodgement elevation carries.
    sheeting,
    floorAbove,
  };
}

/** Common patio roof sheeting in WA — a starting list; anything can be typed. */
export const PATIO_SHEETING = ["Custom Orb", "Trimdek", "Smartdek", "Smoothdek", "Corrugated"];
...
export function patioColumns(s) {
  ...
  // The attached side abuts the dwelling: no posts along it, and none at its
  // two corners either — the outer beams meet the wall there, they don't stand
  // on posts. So those two corner points are struck out even though the
  // adjacent (standing) sides would otherwise place a shared post on them.
  const wallCorners = new Set();
  if (attachLocal >= 0) {
    const [wa, wb] = PATIO_SIDES[attachLocal];
    wallCorners.add(`${fp[wa].x.toFixed(2)},${fp[wa].y.toFixed(2)}`);
    wallCorners.add(`${fp[wb].x.toFixed(2)},${fp[wb].y.toFixed(2)}`);
  }
  const posts = [];
  for (let k = 0; k < 4; k++) {
  ...
  for (const q of posts) {
    ...
    if (!seen.has(key) && !wallCorners.has(key)) seen.set(key, q);
  }
  return [...seen.values()];
}
...
/**
 * A patio elevation, reduced to the few numbers a view needs to be drawn.
 *
 * `span` is the face being looked at: "w" is the width face, "d" the depth
 * face. Everything is in the patio's own local frame — an elevation is about
 * the structure, so the lot's rotation never enters into it.
 *
 * The slope shows as a true rake in the view whose horizontal axis it runs
 * along, and runs into the page in the other. `postXs` are the posts you see
 * across that face — the busier of the two sides parallel to the view, with
 * the side that carries the dwelling wall contributing none.
 */
export function patioElevationProfile(s, span) {
  const p = sanitisePatio(s.patio);
  const h = patioRoofHeights(s);
  const width = span === "d" ? s.d : s.w;
  const slopeAxis = (p.fall % 2 === 0) ? "d" : "w";
  const sides = span === "w" ? [0, 2] : [1, 3];
  const attachLocal = p.mount === "attached" ? p.attach : -1;
  // The dwelling wall shows in the elevation where the attached edge is seen
  // END-ON — the PERPENDICULAR view — standing at one end, not in the view
  // parallel to it. A horizontal attach edge (0/2) is seen end-on in the depth
  // view; a vertical one (1/3) in the width view.
  const attachHere = attachLocal !== -1 && (span === "w"
    ? (attachLocal === 1 || attachLocal === 3)
    : (attachLocal === 0 || attachLocal === 2));
  const n = Math.max(0, ...sides.map((k) => (k === attachLocal ? 0 : p.cols[k])));
  let postXs = [];
  for (let i = 0; i < n; i++) {
    postXs.push(r4(width * (n === 1 ? 0.5 : i / (n - 1))));
  }
  // In the end-on (perpendicular) view of the attached edge the beams meet the
  // dwelling wall at one end, so no post stands at that corner. The wall sits
  // at x = 0 for a top/left attachment (0/3), at x = width for a bottom/right
  // one (1/2).
  if (attachHere) {
    const wallAt = attachLocal === 0 || attachLocal === 3 ? 0 : r4(width);
    postXs = postXs.filter((x) => Math.abs(x - wallAt) > 1e-6);
  }
  return {
    span,
    width: r4(width),
    ...
    ridge: h.ridge,
    rise: h.rise,
    slopeInPlane: span === slopeAxis,
    // Fall sides 0 (top) and 3 (left) sit at x = 0; 1 (right) and 2 (bottom)
    // at x = width. So the low (eave) end is the start for 0 and 3.
    lowAtStart: p.fall === 0 || p.fall === 3,
    attachHere,
    attachAtStart: attachLocal === 0 || attachLocal === 3,
    postXs,
    // Lodgement figures: the sheeting material, whether this face carries a
    // gutter (the low/eave sides — never the wall side), and the patio floor's
    // height above natural ground.
    sheeting: p.sheeting,
    floorAbove: p.floorAbove,
    gutterHere: patioGutter(s).sides.some((k) => (span === "w" ? (k === 0 || k === 2) : (k === 1 || k === 3))),
  };
}
...
// ---------------------------------------------------------------------------
// The house-plan underlay — screen only, never printed, never uploaded.
//
// A second trace-over guide beside the aerial: the client's own house plan,
// a PDF page or an image, sitting behind the drawing so they can trace the
// building over it. Unlike the aerial it is not geo-referenced — it is just a
// picture the client places, turns and scales by hand until it sits true —
// and unlike the aerial it is theirs. So the picture itself never leaves the
// browser (it lives in IndexedDB on their machine); only this placement,
// which is a handful of numbers, travels with the saved design.
//
// `mpp` is metres of real ground per image pixel — the scale the client sets
// by drawing a line along something they know the length of. `cx`/`cy` is the
// image centre in plan metres, `rot` its own free turn in degrees (the sheet's
// north never enters into it — it isn't a map). `w`/`h` are the picture's
// natural pixel size, kept so its on-plan size can be worked out and so a
// re-added picture of the same size lands back where it was.
// ---------------------------------------------------------------------------

export const PLAN_UNDERLAY_MIN_MPP = 0.00001;
export const PLAN_UNDERLAY_MAX_MPP = 10;

/** Read a house-plan underlay's placement back off a saved design. Anything
 *  that isn't a fully placed picture comes back as "nothing placed", which is
 *  every design saved before this existed and every one with no house plan. */
export function sanitisePlanUnderlay(raw) {
  const u = raw && typeof raw === "object" ? raw : {};
  const num = (v, fb) => (typeof v === "number" && Number.isFinite(v) ? v : fb);
  const w = Math.round(num(u.w, 0));
  const h = Math.round(num(u.h, 0));
  const mpp = num(u.mpp, 0);
  const placed = u.placed === true && w > 0 && h > 0 && mpp > 0;
  if (!placed) {
    return {
      placed: false, cx: 0, cy: 0, mpp: 0, rot: 0,
  ...
  return {
    placed: true,
    cx: Math.round(num(u.cx, 0) * 1000) / 1000,
    cy: Math.round(num(u.cy, 0) * 1000) / 1000,
    mpp: Math.min(Math.max(mpp, PLAN_UNDERLAY_MIN_MPP), PLAN_UNDERLAY_MAX_MPP),
    rot: ((Math.round(num(u.rot, 0) * 10) / 10) % 360 + 360) % 360,
    opacity: clampUnderlayOpacity(u.opacity),
    visible: u.visible !== false,
    locked: u.locked !== false,
    w, h,
    page: Math.max(1, Math.round(num(u.page, 1))),
  };
}

/** Rescale a placed picture so a span the client just drew across it — `nowM`
 *  metres at the current scale — becomes its true length `realM`, keeping the
 *  picture's centre where it is. Returns the new metres-per-pixel, clamped, or
 *  the old one if the input makes no sense. */
export function rescalePlanUnderlay(mpp, nowM, realM) {
  const m = Number(mpp), n = Number(nowM), r = Number(realM);
  if (!(m > 0) || !(n > 0) || !(r > 0)) return m > 0 ? m : 0;
  return Math.min(Math.max((m * r) / n, PLAN_UNDERLAY_MIN_MPP), PLAN_UNDERLAY_MAX_MPP);
}
