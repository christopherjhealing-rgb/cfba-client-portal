...
export function boundaryRow(lot) {
  ...
  return o === "cadastre"
    ? `${id}${src}${got}`
    : `${id}${src}${got}; adjusted by hand by the applicant`;
}
...
export function boundaryFooter(lot) {
  ...
  if (o === "cadastre-adjusted") {
    return "Prepared by the applicant using the CFBA plan tool. The lot " +
      `boundary was taken from ${src}${on} — a cadastral boundary, indicative ` +
      "rather than surveyed — and has since been adjusted by hand by the " +
      "applicant, so what is drawn is the applicant's own measurement and no " +
      "longer that record. Structures, dimensions and the nominated frontage " +
      "are as entered by the applicant. Not a certified document and not a " +
      "survey.";
  }
  if (o === "traced") {
    return "Prepared by the applicant using the CFBA plan tool. The lot " +
