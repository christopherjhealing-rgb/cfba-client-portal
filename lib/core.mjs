...
//
// The board carries a column called "Send?" with a date partner called "Job
// Sent". That pair — not the main Status column, which tracks the assessment
// and has no Sent label at all — is where a job's journey to the client is
// recorded. An earlier version of this code wrote to Status and silently did
// nothing for its whole life.
//
// The column is a ladder. The office drives the first rungs by hand; the
// portal drives the last two, because they are the only two it can see:
//
//   NO / YES     retired. See below — they stay on the ladder anyway.
//   SENT         the office issued it and put the files out. Set by hand.
//   READY        the portal HAS the files and the client has been emailed.
//   DOWNLOADED   the client has actually taken it.
//
// The gap between SENT and READY is the one worth having. It is exactly where
// job 56733 sat for a night: the card said issued, the files were in the
// folder, and nothing anywhere said the client still couldn't get them.
//
// NO and YES were a pre-flight flag the office stopped needing, and the labels
// came off the board in Aug 2026. They stay on the ladder deliberately. A card
// that still carries one — Monday leaves old values behind, and a 3,700-item
// board has corners nobody has looked in for years — must still be able to
// move forward. Take them out and sendRank returns -1 for those cards, which
// reads as "somebody put that there on purpose", and the portal leaves them
// alone: a job stranded, silently, by a tidy-up. They cost nothing to keep.
//
// One rule, and it is the whole safety story: the portal only ever moves a
// card FORWARD along the ladder. It never rewrites a rung already reached,
// never drags a card back, and never invents a label —
// `create_labels_if_missing` is off for every write in lib/monday.ts, because
// a portal that quietly adds labels to a 3,700-item board is a portal nobody
// can trust. A rung the board doesn't carry yet fails loudly in the admin
// banner and the daily report, rather than appearing on the board unasked.
// ---------------------------------------------------------------------------

/** The rungs the office sets by hand. The portal reads them, never writes.
 *  NO and YES are retired but kept — see the note above before removing them. */
export const SEND_OFFICE_RUNGS = ["NO", "YES", "SENT"];
/** The two the portal writes. Spelling is overridable in lib/env.ts, because
 *  these labels are typed onto the board by a human, not created by us. */
export const SEND_READY = "READY";
export const SEND_DOWNLOADED = "DOWNLOADED";

export function sendLadder(ready = SEND_READY, downloaded = SEND_DOWNLOADED) {
  return [...SEND_OFFICE_RUNGS, ready, downloaded];
}

/** Where a label sits on the ladder; -1 for blank, or anything we don't know. */
export function sendRank(label, ladder = sendLadder()) {
...
/**
 * Whether to move "Send?" to `target`, given what it says right now.
 *
 *   "write"          — go ahead
 *   "already"        — the card is at that rung or past it; leave it
 *   "unknown"        — the column says something not on the ladder. Somebody
 *                      put it there on purpose; that's a decision, not a gap.
 *   "unknown-target" — we were asked for a rung that isn't on the ladder.
 *                      Caller's bug; refuse rather than guess.
 */
export function sendColumnWrite(currentLabel, target, ladder = sendLadder()) {
  const to = sendRank(target, ladder);
  if (to < 0) return "unknown-target";
  const from = sendRank(currentLabel, ladder);
  if (from < 0) return String(currentLabel || "").trim() ? "unknown" : "write";
