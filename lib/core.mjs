...
// ---------------------------------------------------------------------------
// Writing a status BACK to the board. The portal is the client's window onto
// Monday, not a second system of record, so it moves a card only where the
// office would expect it to move and never invents a label to do it.
//
// On download we mark the card Sent. Two guards, both deliberate:
//
//   1. The label has to already exist on the board. `create_labels_if_missing`
//      is off for every write in lib/monday.ts — a portal that quietly adds
//      labels to a 3,700-item board is a portal nobody can trust. If "Sent"
//      isn't there, we log what IS there and leave the card alone.
//   2. The card has to still be at Issued. There is no ordering among Monday
//      labels, so "further along than Sent" isn't something we can compute;
//      what we CAN say is that a card the office has already moved on to
//      To Invoice or Invoiced / Completed must not be dragged back. Requiring
//      the exact status we expect is the conservative reading of that.
// ---------------------------------------------------------------------------
export const SENT_STATUS = "Sent";

/**
 * Whether to move a card to `label` when the client downloads.
 *
 * Pure: hand it the card's current status and the labels the board actually
 * carries, and it returns the decision the caller acts on.
 *
 *   "write"         — go ahead
 *   "no-such-label" — the board has no such label; skip and log
 *   "moved-on"      — the office has already moved the card; leave it
 */
export function downloadStatusWrite(currentStatus, boardLabels, label = SENT_STATUS) {
  const labels = boardLabels instanceof Set ? boardLabels : new Set(boardLabels || []);
  if (!labels.has(label)) return "no-such-label";
  if (currentStatus !== READY_STATUS) return "moved-on";
  return "write";
}
