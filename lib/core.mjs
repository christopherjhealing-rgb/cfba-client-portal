...
export const PORTAL_LODGED = "LODGED";
...
/**
 * LODGED is the lowest rung: the portal stamps it the instant it creates a
 * card, so the board shows at a glance which jobs came through the portal —
 * before they're issued, which is exactly when the office needs to know (an
 * FIR reply reaches a portal client, but an email client has to be emailed).
 * Being on the ladder is what lets a LODGED job climb normally to ISSUED and
 * on; if it weren't a recognised rung, the forward-only write would refuse to
 * move it and the job would sit at LODGED for ever.
 */
export function portalLadder(
  issued = PORTAL_ISSUED, ready = PORTAL_READY, downloaded = PORTAL_DOWNLOADED,
  lodged = PORTAL_LODGED
) {
  return [lodged, issued, ready, downloaded];
}
 ...
 *   cancelled   -> terminal; a Past state everywhere (never "in progress",
 *                  never counted as running — DESIGN.md §12)
 */
export function jobBucket(job, now = new Date(), months = 6) {
  if (statusKey(job.mondayStatus) === statusKey(CANCELLED_STATUS)) return "cancelled";
...
export function groupJobs(jobs, now = new Date(), months = 6) {
  const out = { ready: [], in_progress: [], downloaded: [], expired: [], cancelled: [] };
...
/**
 * Lodgements that should still show as their own "received" rows in the
 * client's job list, because the jobs table has nothing for them yet.
 *
 * Two kinds. A PENDING submission is waiting for the office to accept it. An
 * ACCEPTED one is already a card on the board — but its job row only exists
 * after the office gives the card a reference and the sync picks it up, which
 * can be hours. In between, the job the client just lodged must not vanish
 * from their list: that gap is exactly when they look.
 *
 * The row retires the moment a job carrying the submission's Monday item id
 * appears. Amendments never show (they belong to their original job), and an
 * accepted submission with no item id is excluded — nothing could ever match
 * it, and a row that can never retire would sit there forever.
 */
export function awaitingBoardRows(subs, jobs) {
  const seen = new Set(jobs.map((j) => j.mondayItemId).filter(Boolean));
  return (subs || []).filter((s) => !s.amendmentOf && (
    s.status === "pending" ||
    (s.status === "accepted" && s.mondayItemId && !seen.has(s.mondayItemId))
  ));
}
...
// ---------------------------------------------------------------------------
// Sorting a client's job list. A pure comparator so the same order can be
// produced on the server for the table and proven without one.
// ---------------------------------------------------------------------------
...
const JOB_SORT_KEYS = new Set(JOB_SORTS.map((s) => s.key));
export const DEFAULT_JOB_SORT = "recent";

/** The number in a reference, for ordering: "56867" -> 56867, "T-1002" ->
 *  1002. NaN when there's no number at all, which sorts to the end. */
function refNumber(job) {
  const m = String(job.ref || "").match(/\d+/);
  return m ? Number(m[0]) : NaN;
}

/** When a job was lodged, as a sortable time. Falls back to issued date, then
 *  to NaN — a job with no date sorts to the end whichever way the list runs. */
function lodgedTime(job) {
  const t = Date.parse(job.receivedAt || job.issuedAt || "");
  return Number.isFinite(t) ? t : NaN;
}

/** Compare two numbers with the missing one (NaN) always last, whatever the
 *  direction — a dateless job shouldn't leap to the top of "oldest". */
function missingLast(a, b, dir) {
  const am = Number.isNaN(a), bm = Number.isNaN(b);
  if (am && bm) return 0;
  if (am) return 1;
  if (bm) return -1;
  return dir * (a - b);
...
/**
 * Order a client's jobs.
 *
 * `actionFirst` floats the jobs waiting on the client (FIR) to the top
 * whatever the sort, so the thing the client has to do is never buried under
 * a re-sort — the list's whole job on those rows is to be seen.
 */
export function sortJobs(jobs, key = DEFAULT_JOB_SORT, { actionFirst = false } = {}) {
  const k = JOB_SORT_KEYS.has(key) ? key : DEFAULT_JOB_SORT;
  const within = (a, b) =>
    k === "oldest" ? missingLast(lodgedTime(a), lodgedTime(b), 1)
      : k === "number" ? missingLast(refNumber(a), refNumber(b), -1)
      : missingLast(lodgedTime(a), lodgedTime(b), -1); // recent
  return [...(jobs || [])].sort((a, b) => {
    if (actionFirst) {
      const d = Number(needsClientInfo(b)) - Number(needsClientInfo(a));
      if (d) return d;
    }
    return within(a, b) || String(a.ref || "").localeCompare(String(b.ref || ""));
  });
}
...
/**
 * The step a job is really at from the client's side.
 *
 * "Issued" on the board is a promise; the certificate has only reached the
 * client once its files are in the portal. Until they are — the "Being
 * finalised" window while the sync settles — the job stays at "Certificate
 * being prepared" rather than ticking Issued, so the timeline never says
 * "done" while the status chip beside it still says "Being finalised". Once
 * the files land (or the job's been downloaded, its files since purged) it's
 * genuinely Issued.
 */
export function effectiveStageIndex(job) {
  const at = stageIndex(job);
  if (at === 4 && (job.fileCount || 0) === 0 && !job.firstDownloadedAt) return 3;
  return at;
}
...
export function stageStates(job) {
  const at = effectiveStageIndex(job);
...
/**
 * Has the client already answered the current information request?
 *
 * The board keeps a job at FIR until the office moves it, which can be a sync
 * cycle after the client has actually sent everything — and a banner that
 * keeps saying "we need something from you" after they've answered is how a
 * client double-sends or rings. The portal records a marker when an FIR-state
 * job gets a reply ({ at, ask: the request text answered }); this says whether
 * that marker answers the CURRENT ask. A new or changed request text means the
 * office has asked again, so the amber state returns.
 */
export function firAnswered(marker, firRequest) {
  if (!marker || !marker.at) return false;
  const askedThen = marker.ask ?? null;
  const askedNow = firRequest ?? null;
  return askedNow === null || askedThen === askedNow;
}
