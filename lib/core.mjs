...
// column plus status: "To Assess" means nobody yet; "To Check" and the Chris
// CDC queues are always Chris; otherwise the named surveyor. Kacie is admin,
// never shown — when only she holds the card the caller keeps the last known
// surveyor (the sync remembers whoever held it before her).
// ---------------------------------------------------------------------------
export function surveyorFor(peopleText, status) {
  const p = (peopleText || "").toLowerCase();
  if (status === "To Assess") return null;
