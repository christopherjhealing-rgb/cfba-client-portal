...
export const PROVENANCE = [
  ...
  "It contains every message sent through the portal on this job, in both " +
    "directions, and lists every document the client attached to a reply.",
  "It does NOT contain: correspondence by email or telephone outside the " +
    ...
    "the client; or the lodged drawings and the issued CDC Package, which are " +
    "filed separately.",
];
 ...
 */
export function transcript({ job, messages, companyName, exportedAt, tz }) {
  ...
  ordered.forEach((m, i) => {
    ...
    for (const f of m.files || []) {
      ...
      files.push({ ...f, path, messageIndex: i + 1 });
    }
  });

  add("heading", "Documents attached to this record");
  if (files.length === 0) {
    add("note", "No documents were attached to any message on this job.");
  } else {
    for (const f of files) {
      add("note", `${f.path} — sent with message ${f.messageIndex}`);
    }
  }
